...
<%--
# 자바빈 (java beans)
1. 웹프로그램에서 데이터의 표현을 효과적으로 처리하기 위하여 사용한다.
2. 구성
...
5. property의 활용..
 ...
 --%>
 <%
 // 1. useBean 사용하지 않게 session을 할당할 때.
...
<%--ex) useBean을 이용하여 Person객체에 이름 나이 사는곳을 저장하고 호출하세요. --%>
<body>
	<h3>useBean 활용</h3>
	<table>
		<tr><th>m2(일반)</th><td><%=m2.getId() %></td></tr>
		<tr><th>m3(useBean)</th>
		<td><jsp:getProperty name="m3" property="id"/></td></tr>

	</table>
<jsp:useBean id="p1" class="jspexp.z01_vo.Person" scope="request"></jsp:useBean>
<jsp:setProperty property="name" name="p1" value="홍길동"/>
<jsp:setProperty property="age" name="p1" value="15"/>
<jsp:setProperty property="loc" name="p1" value="서울 강남"/>
	<h3>useBean 활용(Person)</h3>
	<table>
		<tr><th>p1(ex1 : 이름)</th><td><jsp:getProperty name="p1" property="name"/></td></tr>
		<tr><th>p1(ex1 : 나이)</th><td><%=p1.getAge() %></td></tr>
		<tr><th>p1(ex1 : 사는곳)</th><td><jsp:getProperty name="p1" property="loc"/></td></tr>
	</table>
</body>
</html>