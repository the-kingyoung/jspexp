...
<body>
	...
	<h2>숫자 처리</h2>
	<h3>기본형식: ${num01}</h3>
	<h3>숫자 기본형식 :<fmt:formatNumber value="${num01}" var="false"/></h3>
	...
	<%--
	ex) dao를 이용해서 emplist를 가져와서 forEach 구문으로 아래내용을 출력하되,
	hiredate를 both full형식으로, sal을 천단위 만원으로 표기,
	...
	<table>
		...
		<td><fmt:formatNumber value="${emp.sal}" type="currency" currencySymbol="만원" /></td>
		<td><fmt:formatNumber value="${emp.comm}" type="currency" currencySymbol="만원"/></td>
		<td><fmt:formatNumber value="${emp.comm/emp.sal}" type="percent"/></td></tr>
		</c:forEach>
	</table>
